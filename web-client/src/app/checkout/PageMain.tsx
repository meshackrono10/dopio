  ...
  useEffect(() => {
    const fetchProperty = async () => {
      ...
      try {
        ...
        // Fetch package members if Gold package
        if (response.data.packageGroupId) {
          const pkgResponse = await api.get(`/packages/properties/${propertyId}/package-members`);
          setPackageMembers(pkgResponse.data.properties || []);
        }
      } catch (err) {
        ...
        {showBundleMembers ? (
          <>
            ...
            <div className="space-y-4">
              {packageMembers
                .sort((a: any, b: any) => (a.packagePosition || 0) - (b.packagePosition || 0))
                .map((prop: any) => {
                  const images = Array.isArray(prop.images) ? prop.images :
                    typeof prop.images === 'string' ? JSON.parse(prop.images) : [];

                  return (
                    <div key={prop.id} className="flex flex-col sm:flex-row sm:items-center p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl border border-yellow-200 dark:border-yellow-800">
                      <div className="flex-shrink-0 w-full sm:w-24">
                        <div className="aspect-w-4 aspect-h-3 rounded-xl overflow-hidden relative">
                          <Image
                            ...
                            src={images[0] || "https://images.pexels.com/photos/6373478/pexels-photo-6373478.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=650&w=940"}
                          />
                        </div>
                      </div>
                      <div className="py-2 sm:px-4 space-y-1 flex-grow">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-semibold px-2 py-0.5 bg-yellow-600 text-white rounded">Property {prop.packagePosition}</span>
                        </div>
                        <span className="text-sm font-medium block line-clamp-1">
                          {prop.title}
                        </span>
                        <span className="block text-xs text-neutral-500 dark:text-neutral-400">
                          KES {prop.rent?.toLocaleString()}/month
                        </span>
                      </div>
                    </div>
                  );
