...
const TabFilters = () => {
  ...
  const handleApplyType = () => {
    // Map selected types to layout or other fields if needed
    // For now, we'll just pass it as layout filter if it matches
    filterProperties({ layout: selectedTypes });
    close();
  };

  const handleApplyRooms = () => {
    // Currently PropertyContext might not filter by rooms/beds specifically, 
    // but we can add it or just ignore for now if backend doesn't support it fully in this context
    // Let's assume we can filter by amenities or description for now
    close();
  };
                    ...
                    {typeOfPaces.map((item) => (
                      <div key={item.name} className="">
                        <Checkbox
                          ...
                          onChange={(checked) => {
                            if (checked) {
                              setSelectedTypes([...selectedTypes, item.name]);
                            } else {
                              setSelectedTypes(selectedTypes.filter(t => t !== item.name));
                            }
                          }}
                        />
                      </div>
          ...
          <>
            ...
            >
              <Popover.Panel className="absolute z-10 w-screen max-w-sm px-4 mt-3 left-0 sm:px-0 lg:max-w-md">
                <div className="overflow-hidden rounded-2xl shadow-xl bg-white dark:bg-neutral-900   border border-neutral-200 dark:border-neutral-700">
                  <div className="relative flex flex-col px-5 py-6 space-y-5">
                    <NcInputNumber label="Beds" max={10} onChange={setBeds} />
                    <NcInputNumber label="Bedrooms" max={10} onChange={setBedrooms} />
                    <NcInputNumber label="Bathrooms" max={10} onChange={setBathrooms} />
                  </div>
                  <div className="p-5 bg-neutral-50 dark:bg-neutral-900 dark:border-t dark:border-neutral-800 flex items-center justify-between">
                    <ButtonThird onClick={close} sizeClass="px-4 py-2 sm:px-5">
                      Clear
      ...
      <Popover className="relative">
        {({ open, close }) => (
          <>
            ...
            >
              <span>
                {`KES ${convertNumbThousand(
                  rangePrices[0]
                )} - KES ${convertNumbThousand(rangePrices[1])}`}{" "}
              </span>
              {renderXClear()}
            </Popover.Button>
            <Transition
